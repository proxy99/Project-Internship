...
<table class="table table-striped table-advance table-hover">
                ...
                <?php if(isset($blogs) && is_array($blogs)): ?>
                    <?php foreach($blogs as $blog): ?>
                        <tr style="position: relative;">
                            <td style="color: blue;"><?=$blog->title?></td>
                            <td style="color: blue;"><a href="<?=ROOT?>profile/<?=$blog->user_url?>"><?=$blog->user_data->name?></a></td>
                            <td><?=nl2br(htmlspecialchars(substr($blog->post,0,200)))?></td>
                            <td><img src="<?=ROOT . $blog->image?>" style="width: 150px;"></td>
                            <td style="color: red;"><?=date("jS m Y H:i a",strtotime($blog->date))?></td>
                            <td>
        ...
        <?php elseif($mode == "delete_confirmed"): ?>
            ...
            <div class="status alert alert-success">Your post is deleted successfully</div>
            <a href="<?=ROOT?>admin/blogs">
                <input type="button" class="btn btn-success pull-right" value="Back to blog posts">
            </a>
        ...
        <?php elseif($mode == "delete" && is_object($blogs)): ?>
            ...
            <tbody>  
                <tr style="position: relative;">
                    <td style="color: blue;"><?=$blogs->title?></td>
                    <td style="color: blue;"><a href="<?=ROOT?>profile/<?=$blogs->user_url?>"><?=$blogs->user_data->name?></a></td>
                    <td><?=$blogs->post?></td>
                    <td><img src="<?=ROOT . $blogs->image?>" style="width: 150px;"></td>
                    <td style="color: red;"><?=date("jS m Y H:i a",strtotime($blogs->date))?></td>
                </tr>
